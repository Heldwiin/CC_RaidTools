...
local function ApplyBarLayout()
    ...
    local size = 30
    local children = {}
    for _, b in ipairs(buttons) do table.insert(children, b) end
    for _, b in ipairs(bar.worldButtons or {}) do table.insert(children, b) end
    if bar.clearButton then table.insert(children, bar.clearButton) end
    ...
    local width, height
    if horizontal then
        width = 4 + count * size + math.max(0, count - 1) * gap
        height = size + 4
    else
        width = size + 4
        height = 4 + count * size + math.max(0, count - 1) * gap
    end
...
local function CreateBar()
    ...
    for i = 1, 8 do
        ...
        b:SetScript("OnEnter", function(self)
            ...
            GameTooltip:SetText(RAID_TARGET_ICON and RAID_TARGET_ICON[i] or "Marque de raid")
            GameTooltip:AddLine("Clic gauche : poser / retirer", 0.75, 0.75, 0.75)
            GameTooltip:AddLine("Clic droit : retirer", 0.75, 0.75, 0.75)
            GameTooltip:Show()
            self.bg:SetColorTexture(C.BRAND_R * 0.22, C.BRAND_G * 0.22, C.BRAND_B * 0.30, 0.98)
        end)
        b:SetScript("OnLeave", function(self)
    ...
    for i = 1, 8 do
        ...
        b:SetScript("OnEnter", function(self)
            GameTooltip:SetOwner(self, "ANCHOR_TOP")
            GameTooltip:SetText("Marqueur au sol " .. i)
            GameTooltip:AddLine("Clic gauche : placer", 0.75, 0.75, 0.75)
            GameTooltip:AddLine("Clic droit : retirer les marqueurs", 0.75, 0.75, 0.75)
            GameTooltip:Show()
            self.bg:SetColorTexture(C.BRAND_R * 0.22, C.BRAND_G * 0.22, C.BRAND_B * 0.30, 0.98)
        end)
        b:SetScript("OnLeave", function(self)
    ...
    local cross = clear:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    cross:SetPoint("CENTER", 0, 0)
    cross:SetText("×")
    cross:SetTextColor(0.9, 0.9, 0.9)
    clear:SetScript("OnClick", ClearWorldMarkers)
    clear:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_TOP")
        GameTooltip:SetText("Retirer les marqueurs au sol")
        GameTooltip:Show()
        self.bg:SetColorTexture(C.BRAND_R * 0.22, C.BRAND_G * 0.22, C.BRAND_B * 0.30, 0.98)
    end)
...
local function BuildUI(parent)
    ...
    local enable = CreateFrame("CheckButton", nil, parent)
    ...
    local lock = CreateFrame("CheckButton", nil, parent)
    lock:SetSize(48, 24)
    ...
    local hbg = horizontal:CreateTexture(nil, "BACKGROUND"); hbg:SetAllPoints(); horizontal.bg=hbg
    hbg:SetColorTexture(0.045,0.045,0.055,0.94)
    local htext = horizontal:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); htext:SetPoint("CENTER"); htext:SetText("Horizontal"); horizontal.text=htext
    local vertical = CreateFrame("Button", nil, parent)
    vertical:SetSize(110,26); vertical:SetPoint("LEFT",horizontal,"RIGHT",6,0)
    local vbg=vertical:CreateTexture(nil,"BACKGROUND"); vbg:SetAllPoints(); vertical.bg=vbg; vbg:SetColorTexture(0.045,0.045,0.055,0.94)
    local vtext=vertical:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); vtext:SetPoint("CENTER"); vtext:SetText("Vertical"); vertical.text=vtext
    local function RefreshOrientation()
        local h = db.orientation == "HORIZONTAL"
        horizontal.bg:SetColorTexture(h and C.BRAND_R*0.28 or 0.045,h and C.BRAND_G*0.28 or 0.045,h and C.BRAND_B*0.30 or 0.055,0.95)
        vertical.bg:SetColorTexture(not h and C.BRAND_R*0.28 or 0.045,not h and C.BRAND_G*0.28 or 0.045,not h and C.BRAND_B*0.30 or 0.055,0.95)
    end
