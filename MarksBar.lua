...
-- SavedVariables are loaded after Lua files execute. Resolve the DB on ADDON_LOADED.
local db
...
local function MakeIconButton(parent, width, height)
    ...
    b:SetSize(width, height); b:SetMouseClickEnabled(true); b:RegisterForClicks("AnyUp", "AnyDown")
...
local function AddTooltip(b, title, line1)
    b:SetScript("OnEnter", function(self) GameTooltip:SetOwner(self, "ANCHOR_TOP"); GameTooltip:SetText(title); if line1 then GameTooltip:AddLine(line1, 0.75, 0.75, 0.75) end; GameTooltip:Show(); self.bg:SetColorTexture(C.BRAND_R * 0.22, C.BRAND_G * 0.22, C.BRAND_B * 0.30, 0.98) end)
...
local function CreateRowBackground(parent)
    local bg = parent:CreateTexture(nil, "BACKGROUND"); bg:SetTexture("Interface\\Buttons\\WHITE8X8"); bg:SetVertexColor(0.015, 0.015, 0.02, 0.90); return bg
end
...
local function ApplyDisplayMode()
    ...
    if db.enabled then
        bar:Show()
        if db.mouseoverDisplay then bar:SetAlpha(bar:IsMouseOver() and (db.alpha or 1) or 0) else bar:SetAlpha(db.alpha or 1) end
    else bar:Hide() end
end
...
local function CreateBar()
    ...
    bar:SetBackdrop({bgFile="Interface\\Buttons\\WHITE8X8", edgeFile="Interface\\Buttons\\WHITE8X8", edgeSize=1}); bar:SetBackdropColor(0,0,0,0); bar:SetBackdropBorderColor(0,0,0,0)
...
local function BuildUI(parent)
    ...
    local mouse=CreateFrame("CheckButton",nil,parent,"BackdropTemplate"); mouse:SetSize(48,24); mouse:SetPoint("TOPLEFT",lock,"BOTTOMLEFT",0,-12); C.SkinCheckBox(mouse); mouse:SetChecked(db.mouseoverDisplay); mouse:SetScript("OnClick",function(self) SetMouseoverDisplay(self:GetChecked()) end)
    local mouseText=parent:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); mouseText:SetPoint("LEFT",mouse,"RIGHT",8,0); mouseText:SetText("Afficher uniquement au mouseover")
...
events:SetScript("OnEvent",function(_,event,arg1)
    if event=="ADDON_LOADED" and arg1=="CC_RaidTools" then
        ...
        db.scale=db.scale or 1; db.alpha=db.alpha or 1; db.point=db.point or "CENTER"; db.relativePoint=db.relativePoint or "CENTER"; db.x=db.x or 0; db.y=db.y or -180
        CreateBar(); SetLocked(db.locked); ApplyBarLayout(); ApplyDisplayMode()
