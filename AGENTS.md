...
The popup requires an explicit click ("Ok") to dismiss, or auto-closes after 15s (`autoCloseTimer`, cancelled on manual dismiss so it never fires against a fresh reminder shown right after) — a passive chat message would be too easy to miss right before a pull, but it also shouldn't sit there indefinitely if the player is mid-pull and never gets back to it.
