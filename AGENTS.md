...
The popup requires an explicit click ("Ok") to dismiss, or auto-closes after `REMINDER_DURATION` (15s) — a passive chat message would be too easy to miss right before a pull, but it also shouldn't sit there indefinitely if the player is mid-pull and never gets back to it. The countdown is visualized with a `StatusBar` above the confirm button (same `OnUpdate`-driven, 0.03s-throttled pattern as `ReadyCheck.lua`'s own timer bar, reusing the same `atrocity.tga` texture), tied to a `finishAt` timestamp reset on every `ShowReminder()` call so a fresh reminder always gets the full duration rather than continuing a stale countdown.
