...
Spec name lookup uses the *current active* spec (`GetSpecialization`/`GetSpecializationInfo`, same deprecated-API fallback pattern as `BonusRoll.lua`'s loot-spec lookup). The reminder also shows the **loot specialization** (`GetLootSpecDisplay`, same `GetLootSpecialization` lookup as Bonus Roll Confirm's) as its own line, but only when it's *explicitly* set (not just defaulting to the current spec) *and* differs from the current active spec — otherwise it's the same information already shown on the first line.
