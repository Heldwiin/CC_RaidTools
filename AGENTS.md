...
**Currently confirmed broken in live testing (reported by the guild): the module never triggers on a real bonus roll.** Root cause suspected but not yet confirmed: the classic currency-based bonus roll (`BONUS_ROLL_STARTED`/`BonusRollFrame`) appears to have been superseded by a newer "Nebulous Voidcore"/"Voidforge" bonus-loot system in a recent expansion patch — this was found via web research (guides describing an entirely different resource/UX, and a separate currently-maintained third-party addon, `VoidcoreAdvisor`, built specifically to add a confirmation overlay to *that* system) but the exact new event/frame name could not be confirmed from available sources. A `/ccrtbonusdebug` slash command was added (sniffs every event containing ROLL/VOID/BONUS for 90s, prints matches to chat) specifically to gather real data next time this happens in-game, rather than guessing at another frame path blind. Once the real event/frame is identified this way, rewire `OnBonusRollStarted`/`GetBonusRollButtons` to it and remove the `/ccrtbonusdebug` sniffer.
