...
- The pool never dumps the whole guild roster directly (large guilds can have hundreds of members) — a "Choisir..." button opens a picker popup with a search box, per-rank filter chips (built dynamically from whatever ranks are actually present) and a scrollable member list. Only explicitly-picked names (`CCRaidToolsDB.raidGroups.guildPicks`) show up in the pool; "select/deselect all" in the picker only affects whatever the current search/rank filter matches.
