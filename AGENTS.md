...
`StyleModuleButton`'s `iconBackground` texture is a positioning anchor only (`SetColorTexture(0, 0, 0, 0)`, fully transparent) — it used to be a visible near-black square, which peeked out past the icon's circular edge as visible black corners once the icon art started providing its own opaque circular background. Don't give it a visible color again unless the icon art changes to need a literal square backing.
