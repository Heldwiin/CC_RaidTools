...
- `AutoLog.lua` — automatic combat logging for configured raid/dungeon difficulties, plus world bosses. World bosses aren't reliably detectable via `GetInstanceInfo()`'s instanceType/difficultyID — reported missing from a guild member's logs, first assumed to be a `/reload` issue, but turned out to be a genuine detection gap: Normal/Heroic of the same encounter logged fine via the standard raid path (so it does report `instanceType == "raid"`), but Mythic didn't, meaning that tier reports an unexpected `difficultyID` for this specific encounter. Turned out to be a "Lair" (the newer instanced world-boss format) running the newly-introduced Flexible Mythic system (15-25 players, first seen on the Sporefall raid in patch 12.0.7) — a genuinely new difficulty type, likely with its own non-standard `difficultyID` that couldn't be confirmed via research (too new/niche to be indexed anywhere with that level of detail). Detected separately by target classification instead, deliberately avoiding the need to know that ID at all: once the player targets a `UnitClassification(unit) == "worldboss"` unit, `inWorldBossEncounter` stays true for the rest of the encounter (so tabbing off it mid-fight to heal someone doesn't immediately stop the log) until `ZONE_CHANGED_NEW_AREA` resets it. **Important**: this classification check in `IsLoggingTarget()` must always run, never inside an early `return` gated on `instanceType == "raid"`/`"party"` matching — the first version of this fix had exactly that bug, making the fallback unreachable whenever instanceType happened to match raid/party at all, even with a non-matching difficultyID (i.e. it would have silently failed to fix the reported Mythic case). No dedicated "world boss" toggle in settings — since which specific difficulty tier a non-standard-ID world boss encounter is at can't be reliably determined, the classification fallback is instead gated on `db.normal or db.heroic or db.mythic` (logs if *any* of those raid checkboxes is enabled, matching the general "I care about this content" intent they already express) rather than adding separate UI for it. Verified with a standalone simulation of both the working Normal/Heroic path and the broken-then-fixed Mythic-with-unusual-ID path. Reported still not logging even after this fix and the classification-based fallback — `/ccrtlogdebug` slash command added (toggles verbose prints in `IsLoggingTarget`, `CheckWorldBossTarget`, `CheckAutoLog` showing instanceType/difficultyID/checkbox states/`UnitClassification("target")`/final decision) to get real data instead of guessing at a third fix blind. **Root cause confirmed by the guild directly using that debug output**: Flexible Mythic's `difficultyID` is `233` (`DIFFICULTY_MYTHIC_FLEX`), now checked explicitly alongside `DIFFICULTY_MYTHIC` (16) in the standard raid path — this covers the reported case directly, no longer relying solely on the classification fallback for it. The classification-based fallback stays in place regardless, as a safety net for any *other* future content with a similarly unusual `difficultyID` that hasn't been identified yet. `/ccrtlogdebug` was left in (unlike other one-off debug additions in this addon that got removed once their bug was found) since a live `difficultyID` value is exactly the kind of thing likely to matter again for future content — don't remove it without a specific reason to.
...
- The pool never dumps the whole guild roster directly. Instead it's hard-filtered to a fixed, small set of guild rank indices (`ALLOWED_GUILD_RANKS` at the top of the guild-source section) matching this guild's raiders/officers/applicants ranks — everyone in one of those ranks shows up automatically as soon as Guilde is selected, no manual per-member picking. Adjust `ALLOWED_GUILD_RANKS` if the guild's rank structure changes (remember `GetGuildRosterInfo`'s `rankIndex` is 0-based, while the in-game guild UI shows "Grade 1".."Grade 9" 1-based).
...
- The module's own panel lists everyone seen (self included, tagged), colored by whether they're outdated, current, or ahead — this is the officer-facing view; the reminder above is the self-diagnostic one. Both read from the same `known` table, but the panel additionally filters `known` down to `GetCurrentRosterNames()` (current raid/party members, self always included) at display time — `known` itself is never pruned (harmless small cache, and lets someone reappear instantly without a fresh broadcast if you regroup with them later), but showing everyone ever seen this session regardless of current group membership was reported as a bug (guild members lingering in the list long after they'd left the group).

## Bonus Roll Confirm

`BonusRoll.lua` prompts for confirmation before a bonus roll (Roll or Pass) actually goes through, and shows the loot specialization it will be awarded for. Inspired by a small standalone addon the guild uses (its README/CHANGELOG were read first to understand the feature; its actual source was later shared by the guild after the first implementation shipped broken — reviewed for the *technique* it uses, reimplemented independently in this addon's own style rather than copied).

**Hard constraint, verified against a Blizzard forum report before the first version of this module was built**: the bonus roll Roll button is tied to a protected action (rolling casts a real spell under the hood). Redirecting a secure button's `clickbutton` attribute at `BonusRollFrame.PromptFrame.RollButton` to simulate the click throws `ADDON_ACTION_BLOCKED` — an addon cannot click it for the player that way. That constraint still holds; what changed is *how* to work within it.

**First version (shipped broken, replaced) disabled instead of clicking**: on `BONUS_ROLL_STARTED` it disabled the native buttons, showed a popup, and re-enabled them on confirm — the player still had to click the real button a second time. This shipped in 1.2.13+ and was reported non-functional on a real bonus roll in live testing (the settings panel's Tester button worked fine, confirming the popup UI itself wasn't the problem).

**Current version hooks the native buttons' OnClick instead** (`HookButton`/`TryHookBonusRollFrame`): captures each button's existing `OnClick` handler, replaces it with one that shows the confirmation popup first and only calls the captured original if the player confirms. This still only ever runs the real action as a consequence of a genuine click (first on the native button, then on our own confirm button) — no secure-template click simulation, so it doesn't hit `ADDON_ACTION_BLOCKED`. One click on Roll/Pass, one click on Confirm, done — no more "confirm, then go click the real button again." Hooking is idempotent per-button (`_ccrtBonusHooked` flag) and triggered from two places for redundancy: `BONUS_ROLL_STARTED` (with a 0.1s buffer in case `PromptFrame`'s buttons aren't populated yet) and `hooksecurefunc("BonusRollFrame_StartBonusRoll", ...)`.
...
Spec name lookup uses the *current active* spec (`GetSpecialization`/`GetSpecializationInfo`, same deprecated-API fallback pattern as `BonusRoll.lua`'s loot-spec lookup). The reminder also shows the **loot specialization** (`GetLootSpecDisplay`, same `GetLootSpecialization` lookup as Bonus Roll Confirm's) as its own line, always — this was initially suppressed when it matched the current spec exactly (to avoid duplicate info), but the guild wanted it shown every time regardless, so the comparison was dropped.
...
The popup can be dismissed via a close-X button (top-right, same pattern/texture — `Close.png` — as the main CC RaidTools window's own close button), or it auto-closes after `REMINDER_DURATION` (15s) — a passive chat message would be too easy to miss right before a pull, but it also shouldn't sit there indefinitely if the player is mid-pull and never gets back to it. There's no separate "confirm" button; closing it (manually or via timeout) is the only interaction. The countdown is visualized with a `StatusBar` at the bottom of the popup (same `OnUpdate`-driven, 0.03s-throttled pattern as `ReadyCheck.lua`'s own timer bar, reusing the same `atrocity.tga` texture), tied to a `finishAt` timestamp reset on every `ShowReminder()` call so a fresh reminder always gets the full duration rather than continuing a stale countdown.

The popup is movable (`SetMovable`/`EnableMouse`/drag handlers, standard pattern) and its position **persists across reloads and relogs** — saved to `db.framePoint`/`frameRelativePoint`/`frameX`/`frameY` on `OnDragStop`, and used instead of the default `CENTER` anchor on the next `EnsureConfirmFrame()` call (which now calls `InitDB()` itself defensively, since it can run before any trigger event's own `InitDB()` call the first time).

The popup features a prominent mascot portrait, picked at random from a pool (`MASCOTS` table at the top of `SpecReminder.lua`, files `TexturesGUI/SpecReminderMascot1.png` … `SpecReminderMascotN.png`, all guild-supplied images). `PickMascot()` never returns the same image twice in a row when the pool has more than one entry, and `ShowReminder()` only re-rolls it when the popup isn't already visible (a Ready Check firing right after zone entry keeps the current image). To add a mascot: convert it to a 256x256 PNG with a transparent background (source images are usually webp with alpha — Windows' built-in WIC decoder drops the alpha channel, so render through headless Chrome instead), drop it in `TexturesGUI`, and append it to `MASCOTS`. New texture files are only picked up after a full client restart, not `/reload`. Full opacity, `ARTWORK` layer, positioned in its own right-hand column with the title/body text anchored to its left. Current state: 340x155 popup, 150x150 mascot; check the file directly rather than trusting exact pixel values here if it's been touched again since.
...
- Everything below is **off by default** (opt-in, the addon ships to the whole guild):
  - **Invites** (`acceptInvites`): `PARTY_INVITE_REQUEST` (name, tank, healer, damage, isXRealm, allowMultipleRoles, inviterGUID, isQuestSessionActive — `Blizzard_Game/Mainline/EventImplementation.lua`). Only plain invites (no role request → no `LFGInvitePopup`, no quest session), not already grouped, not when `WillAcceptInviteRemoveQueues()`, inviter GUID readable (not secret) and a BNet friend / friend / guild member. Accepted by clicking button 1 of the `PARTY_INVITE` StaticPopup one frame later. **Never** `AcceptGroup()` + hide the popup: its `OnHide` calls `DeclineGroup()` unless its own accept flag was set.
  - **Resurrect** (`acceptResurrect`): clicks button 1 of whichever of `RESURRECT` / `RESURRECT_NO_SICKNESS` / `RESURRECT_NO_TIMER` is shown, retrying once a second (up to 10s) because `RESURRECT` starts with Accept disabled for the corpse recovery delay. Skipped while the fight is live: our own `PLAYER_REGEN_*` flag, `IsEncounterInProgress()`, or the caster in combat; a secret caster name counts as live.
  - **Fast loot** (`fastLoot`): on `LOOT_READY`, `LootSlot` every slot, only when autoloot applies (`autoLootDefault` CVar XOR `AUTOLOOTTOGGLE` modifier), not fishing, 0.3s throttle.
  - **Merchant** (`sellJunk`, `autoRepair`, `repairGuildFunds`): `C_MerchantFrame.IsSellAllJunkEnabled`/`GetNumJunkItems`/`SellAllJunkItems`; `CanMerchantRepair`, `GetRepairAllCost`, `RepairAllItems(useGuild)`, `CanGuildBankRepair`, then a 0.6s re-check that pays any remainder personally. The repair cost can be a secret value — only compared/printed when `issecretvalue` says it isn't. Shift at the merchant skips both.
  - **Quests** (`questAccept`, `questTurnIn`): `GOSSIP_SHOW` (turn-ins first via `C_GossipInfo.SelectActiveQuest`, then non-trivial non-ignored `SelectAvailableQuest`), `QUEST_GREETING` (index-based globals), `QUEST_DETAIL` (same branch as `QuestDetailAcceptButton_OnClick`: `AcknowledgeAutoAcceptQuest` for auto-accept quests else `AcceptQuest`; PvP-flagged quests skipped since they need a confirm popup), `QUEST_PROGRESS` (`CompleteQuest` if completable), `QUEST_COMPLETE` (`GetQuestReward(0 or 1)`; skipped with several reward choices or a money cost). Gossip *options* are never auto-selected.
  - **Duels / pet duels** (`declineDuels`, `declinePetDuels`): button 2 of `DUEL_REQUESTED` / `PET_BATTLE_PVP_DUEL_REQUESTED` unless the challenger is a character or BNet friend; an unreadable name leaves the popup alone.
  - **Cinematics** (`skipCinematics`): `CINEMATIC_START(canBeCancelled)` → `StopCinematic()`, else `CancelScene()` only if `CanCancelScene()` — deliberately without `CinematicFrame_CancelCinematic`'s last-resort `VehicleExit()`. `PLAY_MOVIE` → `MovieFrame:FinishMovie()` next frame (what its own skip-confirm button calls).
  - **Talking head** (`hideTalkingHead`): `hooksecurefunc(TalkingHeadFrame, "PlayCurrent")` → hide the frame; the voice line keeps playing.
...
All 11 modules use commissioned custom artwork (`TexturesGUI/<Module>.png`) in a shared "epic CC RaidTools" style: radiant purple/gold glow, beveled gold ring border, bold clean silhouette per module (crown, scroll+quill, checkmark, three raid-group figures, envelope+arrow, crosshair, raid-marker cluster, armored knight+gear, shield+checkmark, two dice, bell for Spec Reminder). The panel-header icon (`ApplyPanelIcon` in `ModuleIcons.lua`) renders it at 100px in the corner of each module's own panel.

**The left-hand module menu buttons are text-only, no icon** — after several rounds of icon-sizing/cropping iteration (documented in git history: a tighter `<Module>Menu.png` crop, then a bigger 36px icon, then various margin tweaks), the guild ultimately decided the icons weren't worth the visual complexity in that cramped a space and asked for them removed from the menu specifically, while keeping the full artwork on the panel header where it has room to actually read well. `ApplyModuleIcons`'s menu-button loop hides any `_ccrtModuleIcon`/`_ccrtModuleIconBg` textures left over from a stale saved session and anchors the label straight to the button's left edge (12px padding) instead. Do not resurrect per-button menu icons without being asked — this was a deliberate simplification, not an oversight.

**Important**: the menu button's size is hardcoded in two places that must stay in sync — `StyleModuleButton`'s `SetSize` here, and the anchor spacing (`-62 - (i - 1) * 44`) plus the window's minimum-height floor comment in `CC_RaidTools.lua`'s `BuildMainFrame`/`ResizeMainFrame`. If you resize the menu button again, recompute the spacing (needs to be at least the new button height, plus a few px of gap — 4px worked well) and the floor (last button's bottom edge + ~20px margin) the same way, or buttons will overlap/crowd each other. This has already caused one visible bug in production.
...
- invites: from a guild member / friend (accepted, chat line), from a stranger (popup stays), while queued (popup stays);
- resurrect: a rez after a wipe out of combat is accepted; a battle rez mid-boss and mid-trash is not;
- fast loot with auto loot on, and with the modifier held (manual loot window stays);
- merchant: junk sold, repair with and without guild funds (and guild allowance too low → remainder paid personally), Shift at the merchant does nothing;
- quests: accept / turn in at a gossip NPC, a greeting NPC, a single-quest NPC; a turn-in with several reward choices stays open; grey quests aren't picked up;
- duels and pet duels from a stranger (declined) and a friend (popup stays); skip a real cinematic, a movie, and an in-game scene; talking head hidden but voice heard; delete a rare item (word pre-filled, Yes still needs a click);
