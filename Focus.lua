...
local function CreateSwitchMenu(parent, values, currentValue, onSelect)
    ...
    bg:SetAllPoints()
    bg:SetColorTexture(0.035, 0.035, 0.045, 0.96)
    ...
    for i, entry in ipairs(values) do
        ...
        rowBg:SetAllPoints()
        rowBg:SetColorTexture(0.018, 0.018, 0.024, 1)
        ...
        row:SetScript("OnEnter", function()
            rowBg:SetColorTexture(C.BRAND_R * 0.35, C.BRAND_G * 0.35, C.BRAND_B * 0.48, 1)
        end)
    ...
    button:SetScript("OnEnter", function()
        bg:SetColorTexture(0.07, 0.07, 0.09, 0.98)
    end)
...
local function BuildUI(f)
    ...
    modifierDrop = CreateSwitchMenu(f, modifierValues, modifier, function(value)
        modifier = value
        SaveFocusSettings()
        ApplyFocusBinding()
    end)
    ...
    mouseDrop = CreateSwitchMenu(f, mouseValues, mouseButton, function(value)
        mouseButton = value
        SaveFocusSettings()
        ApplyFocusBinding()
    end)
...
local function RefreshUI()
    if enabledCheck and focusDB then
        ...
        if enabledCheck._ccrtRefresh then enabledCheck:_ccrtRefresh() end
    end
...
events:SetScript("OnEvent", function(_, event, arg1)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        -- Do not enumerate every WoW frame when combat ends.
        -- Apply only the secure override binding; the full unit-frame scan
        -- is performed on addon load and when the focus configuration changes.
        ApplyFocusOverrideBinding()
