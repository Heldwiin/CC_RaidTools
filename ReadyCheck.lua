...
local function StopTicker() if frame and frame.ticker then frame.ticker:Cancel(); frame.ticker=nil end end
local function BuildFrame()
    if frame then return end
    frame=CreateFrame("Frame","CCRaidToolsRaidCheckFrame",UIParent,"BackdropTemplate"); frame:SetSize(625,520); frame:SetPoint("CENTER",320,0); frame:SetMovable(true); frame:EnableMouse(true); frame:RegisterForDrag("LeftButton"); frame:SetScript("OnDragStart",frame.StartMoving); frame:SetScript("OnDragStop",frame.StopMovingOrSizing); C.ApplyPanelSkin(frame)
    frame:SetScript("OnHide",function() StopTicker() end)
    ...
    local close=CreateFrame("Button",nil,frame); close:SetSize(28,28); close:SetPoint("TOPRIGHT",-2,-2); close:EnableMouse(true)
    local closeText=close:CreateFontString(nil,"OVERLAY"); closeText:SetPoint("CENTER",0,1); closeText:SetFont("Fonts\\FRIZQT__.TTF",24,"OUTLINE"); closeText:SetText("×"); closeText:SetTextColor(1,1,1,1); closeText:SetShadowColor(0,0,0,0.8); closeText:SetShadowOffset(1,-1); close:SetScript("OnClick",function() frame:Hide() end); frame.closeButton=close
    ...
    frame._ccrtCountText=H("0/0",4,125,"LEFT"); H("Prêt",130,48); H("Repas",179,48); H("Flacon",228,48); H("Rune",277,48); H("Vantus",326,52); H("Intel",380,38); H("PA",419,38); H("Druid",458,38); H("Endu",497,38); H("Sham",536,38)
...
local function Refresh()
    ...
    local count=IsInRaid() and GetNumGroupMembers() or 0; local ready=0
    for i=1,count do
        local name,_,_,_,_,class=GetRaidRosterInfo(i); local unit="raid"..i; local r=rows[i] or NewRow(frame.child,i); rows[i]=r
        local col=class and RAID_CLASS_COLORS and RAID_CLASS_COLORS[class]; if col then r.nameText:SetTextColor(col.r,col.g,col.b) else r.nameText:SetTextColor(1,1,1) end
        r.nameText:SetText(C.StripRealm(name or "?")); local st=statuses[unit] or (name and (statuses[name] or statuses[C.StripRealm(name)]))
        if GetReadyCheckStatus then local api=GetReadyCheckStatus(unit); if api=="ready" or api=="notready" then st=api; statuses[unit]=api; if name then statuses[name]=api; statuses[C.StripRealm(name)]=api end end end
        r.readyText:SetText(Ready(st)); if st=="ready" then ready=ready+1 end
        local a,b,c,d,e,f,g,h,j=AuraStatus(unit); r.foodText:SetText(OK(a)); r.flaskText:SetText(OK(b)); r.runeText:SetText(OK(c)); r.vantusText:SetText(OK(d)); SetIcon(r.intIcon,e); SetIcon(r.apIcon,f); SetIcon(r.druidIcon,g); SetIcon(r.stamIcon,h); SetIcon(r.shamIcon,j); r:Show()
    end
...
local function Show(starter)
    C.InitDB(); if not AutoPromoteDB.raidCheckEnabled or not IsInRaid() then return end
    wipe(statuses); if starter then statuses[starter]="ready"; local n=UnitExists(starter) and UnitName(starter) or starter; if n then statuses[n]="ready"; statuses[C.StripRealm(n)]="ready" end end
    BuildFrame(); if hideTimer then hideTimer:Cancel(); hideTimer=nil end; frame:Show(); Refresh(); C_Timer.After(.2,Refresh); C_Timer.After(.6,Refresh); C_Timer.After(1.5,Refresh); StopTicker(); frame.ticker=C_Timer.NewTicker(1,function() if frame and frame:IsShown() then Refresh() end end)
end
C.ShowReadyCheck=Show
local function Finish()
    for i=1,(GetNumGroupMembers() or 0) do local u="raid"..i; if UnitExists(u) then local n=UnitName(u); local s=statuses[u] or (n and statuses[n]) or (n and statuses[C.StripRealm(n)]); if s~="ready" and s~="notready" then statuses[u]="notready"; if n then statuses[n]="notready"; statuses[C.StripRealm(n)]="notready" end end end end
    Refresh(); StopTicker(); if hideTimer then hideTimer:Cancel() end; hideTimer=C_Timer.NewTimer(30,function()if frame then frame:Hide() end end)
end
...
local function BuildUI(f)
    local label=f:CreateFontString(nil,"OVERLAY","GameFontNormal"); label:SetPoint("TOPLEFT",f,"TOPLEFT",10,-30); label:SetText("Ready Check :"); label:SetTextColor(C.BRAND_R,C.BRAND_G,C.BRAND_B)
    ...
    local test=CreateFrame("Button",nil,f,"UIPanelButtonTemplate"); test:SetSize(54,22); test:SetPoint("LEFT",chk,"RIGHT",280,0); test:SetText("Test"); C.SkinButton(test); test:SetScript("OnClick",function() if IsInRaid() then wipe(statuses); BuildFrame(); frame:Show(); Refresh() else print("|cff33ff99[CC RaidTools]|r Le test du Ready Check nécessite d'être dans un raid.") end end); f.raidCheckTestButton=test
end
