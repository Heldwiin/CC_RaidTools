...
statuses={}
local ROW_H=20
local FOOD={[308488]=1,[308506]=1,[308434]=1,[308514]=1,[327708]=1,[327706]=1,[327709]=1,[308525]=1,[327707]=1,[308637]=1,[308474]=1,[308504]=1,[308430]=1,[308509]=1,[327704]=1,[327701]=1,[327705]=1,[327702]=1,[382145]=1,[382150]=1,[382146]=1,[396092]=1,[382246]=1,[382247]=1,[382152]=1,[382153]=1,[382157]=1,[382230]=1,[382231]=1,[382232]=1,[382154]=1,[382155]=1,[382156]=1,[382234]=1,[382235]=1,[382236]=1}
...
local function BuildFrame()
    ...
    -- Même bouton de fermeture que la fenêtre principale CCRT : grand X blanc, sans texture Blizzard.
...
local function Show(starter) C.InitDB(); if not AutoPromoteDB.raidCheckEnabled or not IsInRaid() then return end; wipe(statuses); if starter then statuses[starter]="ready"; local n=UnitExists(starter) and UnitName(starter) or starter; if n then statuses[n]="ready"; statuses[C.StripRealm(n)]="ready" end end; BuildFrame(); if hideTimer then hideTimer:Cancel(); hideTimer=nil end; frame:Show(); Refresh(); C_Timer.After(.2,Refresh); C_Timer.After(.5,Refresh); C_Timer.After(1.5,Refresh); if frame.ticker then frame.ticker:Cancel() end; frame.ticker=C_Timer.NewTicker(.25,function()if frame:IsShown() then Refresh() end end) end
local function Finish() for i=1,(GetNumGroupMembers() or 0) do local u="raid"..i; if UnitExists(u) then local n=UnitName(u); local s=statuses[u] or (n and statuses[n]) or (n and statuses[C.StripRealm(n)]); if s~="ready" and s~="notready" then statuses[u]="notready"; if n then statuses[n]="notready"; statuses[C.StripRealm(n)]="notready" end end end end; Refresh(); if frame and frame.ticker then frame.ticker:Cancel(); frame.ticker=nil end; if hideTimer then hideTimer:Cancel() end; hideTimer=C_Timer.NewTimer(30,function()if frame then frame:Hide() end end) end
...
local function BuildUI(f)
    ...
    chk=CreateFrame("CheckButton",nil,f,"BackdropTemplate"); chk:SetSize(48,24); C.SkinCheckBox(chk); chk:SetPoint("TOPLEFT",f,"TOPLEFT",10,-620); local t=chk:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); t:SetPoint("LEFT",chk,"RIGHT",7,0); t:SetText("Check Buffs /appel"); chk:SetScript("OnClick",function(self)AutoPromoteDB.raidCheckEnabled=self:GetChecked() and true or false; if self._ccrtRefresh then self:_ccrtRefresh() end; if not AutoPromoteDB.raidCheckEnabled and frame then frame:Hide() end end); f.raidCheckChk=chk
