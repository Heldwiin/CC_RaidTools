...
local function CountEmptySocketsOnSlot(unit, slot)
    ...
    if gemSocketType then
        ...
        if socketCount > 0 then
            ...
            return emptyCount + fallbackEmpty
        end
    end
...
local function NewRow(parent, i)
    ...
    r.enchantsHover = HoverZone(r.enchantsText, function()
        local data = GetResult(r._ccrtUnit)
        local slots = data and data.missingEnchantSlots or nil
        if not slots or #slots == 0 then return nil end
        local t = { header = C.L.riMissingEnchantTooltip }; for _, s in ipairs(slots) do t[#t+1] = s end; return t
    end)
    r.gemsHover = HoverZone(r.gemsText, function()
        local data = GetResult(r._ccrtUnit)
        local slots = data and data.missingGemSlots or nil
        if not slots or #slots == 0 then return nil end
        local t = { header = C.L.riMissingGemTooltip }; for _, s in ipairs(slots) do t[#t+1] = s end; return t
    end)
    return r
end
