...
-- Enchantable slots for Midnight (12.x).
local ENCHANT_SLOTS = {
    INVSLOT_HEAD,
    INVSLOT_SHOULDER,
    INVSLOT_FINGER1,
...
-- All equipment slots can potentially contain gem sockets.
local ALL_SLOTS = {}
for i = 1, 19 do
...
-- Off-hand enchants only apply to an actual weapon.
...
-- Count genuinely empty sockets from Blizzard's structured tooltip data.
--
-- IMPORTANT: GemSocket is emitted for every socket, including filled sockets.
-- A filled socket also has a GemSocketEnchantment line (and normally gemIcon
-- on the GemSocket line). Therefore counting every GemSocket as "missing"
-- is wrong. We first use gemIcon when available; otherwise we calculate:
--     all sockets - filled socket enchantment lines
--
-- The old EMPTY_SOCKET_* text remains only as a compatibility fallback when
-- Blizzard does not expose structured socket information.
    ...
    if gemSocketType then
        ...
        for _, line in ipairs(data.lines) do
            if line.type == gemSocketType then
                ...
                -- Blizzard's tooltip rules use gemIcon for filled sockets.
                local gemIcon = SafeText(line.gemIcon)
                if gemIcon then
                    filledCount = filledCount + 1
                    sawGemIconField = true
                end
            elseif gemEnchantmentType and line.type == gemEnchantmentType then
                -- Fallback for builds where GemSocket does not expose gemIcon.
                filledCount = filledCount + 1
            end
        end

        if sawStructuredSocket then
            -- If gemIcon was available, it is the most direct indication of
            -- whether each individual socket is occupied. Otherwise the
            -- GemSocketEnchantment lines provide the occupied-socket count.
            if sawGemIconField then
                return math.max(0, socketCount - filledCount)
            end
            return math.max(0, socketCount - filledCount)
        end
    end

    -- Compatibility fallback: only used when structured socket lines are not
    -- available at all. Do not mix this with a structured result.
...
-- This avoids tooltip-cache differences between clients.
...
local function HasEnchantOnSlot(unit, slot)
    ...
    if permanentEnchantType then
        for _, line in ipairs(data.lines) do
            if line.type == permanentEnchantType then return true end
        end
    end
...
local function NewRow(parent, i)
    ...
    r.enchantsHover = HoverZone(r.enchantsText, function()
        ...
        if not data or #data.missingEnchantSlots == 0 then return nil end
    ...
    r.gemsHover = HoverZone(r.gemsText, function()
        local data = results[r._ccrtUnit]
        if not data or #data.missingGemSlots == 0 then return nil end
...
local function RefreshList()
    ...
    if scrollFrame then scrollFrame:SetHeight(math.max(ROW_H, math.min(count * ROW_H, MAX_SCROLL_HEIGHT))) end
...
local function FinalizeInspect(unit, guid)
    ...
    pendingGUID = nil
    pendingQueueIndex = 0
    if unit and UnitExists(unit) and UnitGUID(unit) == guid then results[unit] = CollectUnitData(unit) end
    ClearInspectPlayer()
    RefreshList()
    C_Timer.After(1.8, InspectNext)
end
...
local function StartInspectQueue()
    ...
    wipe(results)
    inspecting = true
    if inspectButton then inspectButton:SetText(C.L.riInspectingButton); inspectButton:Disable() end
