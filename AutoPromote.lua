...
function AutoPromoteUI_RefreshRanks()
    ...
    for i,info in ipairs(list) do
        local r=rankRows[i]
        if not r then r=CreateFrame("CheckButton",nil,mainFrame.rankChild,"BackdropTemplate"); r:SetSize(24,24); C.SkinCheckBox(r); r:SetPoint("TOPLEFT",0,-(i-1)*20); r.text=r:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall"); r.text:SetPoint("LEFT",r,"RIGHT",2,0); r:SetScript("OnClick",function(self) local on=self:GetChecked() and true or false; AutoPromoteDB.ranks[self.rankIndex]=on; AutoPromoteDB.rankNames[self.rankName]=on; if self._ccrtRefresh then self:_ccrtRefresh() end; CheckAndPromote() end); rankRows[i]=r end
