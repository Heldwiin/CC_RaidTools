...
local function RefreshRanksData()
    ...
    for i = 1, (GetNumGuildMembers and GetNumGuildMembers() or 0) do
        ...
        if name and rankIndex ~= nil then
            ...
            if short and not guildRankByShort[short] then
                guildRankByShort[short] = rankIndex
            elseif short then
                guildRankByShort[short] = false
            end
        end
    end
end
...
local function GetGuildRankIndex(name)
    ...
    local short = C.StripRealm(name)
    local fallback = short and guildRankByShort[short]
...
local function CheckAndPromote()
    ...
    for i = 1, GetNumGroupMembers() do
        ...
        if name and rank == 0 then
            ...
            if not should then
                local idx = GetGuildRankIndex(name)
                local rn = idx and discoveredRanks[idx]
...
function AutoPromoteUI_RefreshNames()
    ...
    local sorted = {}
    for n in pairs(AutoPromoteDB.names) do
        table.insert(sorted, n)
    end
    table.sort(sorted)
    if mainFrame.nameChild then
        mainFrame.nameChild:SetHeight(math.max(110, #sorted * 20))
    end
    for i, n in ipairs(sorted) do
        local r = nameRows[i]
        if not r then
            r = CreateFrame("Frame", nil, mainFrame.nameChild)
            r:SetSize(260, 20)
...
function AutoPromoteUI_RefreshRanks()
    ...
    local list = {}
    for idx, n in pairs(discoveredRanks) do
        table.insert(list, { index = idx, name = n })
    end
    ...
    for i, info in ipairs(list) do
        local r = rankRows[i]
        if not r then
            r = CreateFrame("CheckButton", nil, mainFrame.rankChild, "BackdropTemplate")
            r:SetSize(48, 24)
            C.SkinCheckBox(r)
            r:SetPoint("TOPLEFT", 0, -(i - 1) * 26)
            r.text = r:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
            r.text:SetPoint("LEFT", r, "RIGHT", 7, 0)
            r:SetScript("OnClick", function(self)
                local on = self:GetChecked() and true or false
                AutoPromoteDB.ranks[self.rankIndex] = on
                AutoPromoteDB.rankNames[self.rankName] = on
        ...
        r.rankIndex = info.index
        r.rankName = info.name
        r.text:SetText(info.name)
        ...
        r:SetChecked(saved and true or false)
        if r._ccrtRefresh then
            r:_ccrtRefresh()
        end
        r:Show()
    end
    for i = #list + 1, #rankRows do
        rankRows[i]:Hide()
    end
    if mainFrame.rankChild then
        mainFrame.rankChild:SetHeight(math.max(26, #list * 26))
    end
    if mainFrame.rankSection then
        mainFrame.rankSection:SetHeight(math.max(26, #list * 26))
    end
end
...
local function BuildUI(f)
    ...
    add:SetSize(90, 22)
    add:SetPoint("TOPLEFT", input, "BOTTOMLEFT", 0, -8)
    add:SetText("Ajouter")
    C.SkinButton(add)
    add:SetScript("OnClick", function()
        for p in edit:GetText():gmatch("[^,\n]+") do
            local n = C.NormalizeName(p)
            if n then
                AutoPromoteDB.names[n] = true
            end
        end
        edit:SetText("")
        AutoPromoteUI_RefreshNames()
    ...
    listLabel:SetPoint("TOPLEFT", add, "BOTTOMLEFT", 0, -14)
    listLabel:SetText("Joueurs Auto Promote :")
    listLabel:SetTextColor(C.BRAND_R, C.BRAND_G, C.BRAND_B)
    local ns = CreateFrame("ScrollFrame", nil, f, "UIPanelScrollFrameTemplate")
    ns:SetPoint("TOPLEFT", listLabel, "BOTTOMLEFT", 0, -6)
    ns:SetSize(268, 110)
    C.SkinScrollBar(ns)
    local nc = CreateFrame("Frame", nil, ns)
    nc:SetSize(260, 110)
    ns:SetScrollChild(nc)
    f.nameChild = nc
    local rankLabel = f:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    rankLabel:SetPoint("TOPLEFT", ns, "BOTTOMLEFT", -4, -14)
    rankLabel:SetText(C.L.apGuildRankLabel)
    rankLabel:SetTextColor(C.BRAND_R, C.BRAND_G, C.BRAND_B)
    local rc = CreateFrame("Frame", nil, f)
    rc:SetPoint("TOPLEFT", rankLabel, "BOTTOMLEFT", 4, -6)
    rc:SetSize(256, 182)
    f.rankChild = rc
    f.rankSection = rc
    local ng = f:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    ng:SetPoint("TOPLEFT", rc)
    ng:SetText("Pas de guilde, ou liste des rangs en cours de chargement...")
    ng:SetWidth(250)
    ng:SetJustifyH("LEFT")
    f.noGuildText = ng
end
...
for _, ev in ipairs({ "GROUP_ROSTER_UPDATE", "PLAYER_ENTERING_WORLD", "GUILD_ROSTER_UPDATE", "PLAYER_REGEN_ENABLED" }) do
    e:RegisterEvent(ev)
end
