...
function C.SkinCheckBox(box)
    ...
    local left=box:CreateTexture(nil,"BACKGROUND"); left:SetPoint("TOPLEFT",1,-1); left:SetSize(HALF-1,H-2)
    local right=box:CreateTexture(nil,"BACKGROUND"); right:SetPoint("TOPRIGHT",-1,-1); right:SetSize(HALF-1,H-2)
    local divider=box:CreateTexture(nil,"BORDER"); divider:SetPoint("TOP",box,"TOP",0,-1); divider:SetPoint("BOTTOM",box,"BOTTOM",0,1); divider:SetWidth(1); divider:SetColorTexture(0.01,0.01,0.012,1)

    local checkmark=box:CreateTexture(nil,"ARTWORK"); checkmark:SetSize(22,22); checkmark:SetPoint("CENTER",box,"CENTER",12,0); checkmark:SetTexture(CHECK_TEXTURE); checkmark:SetVertexColor(0,0,0,1)
    local crossmark=box:CreateTexture(nil,"ARTWORK"); crossmark:SetSize(22,22); crossmark:SetPoint("CENTER",box,"CENTER",-12,0); crossmark:SetTexture(CROSS_TEXTURE); crossmark:SetVertexColor(0.10,0.10,0.10,1)
    ...
    local function refresh(self)
        ...
        if on then
            left:SetColorTexture(0.30,0.31,0.59,1); right:SetColorTexture(0.38,0.40,0.76,1); checkmark:SetVertexColor(0,0,0,1); checkmark:Show(); crossmark:Hide()
        else
            left:SetColorTexture(0.30,0.30,0.30,1); right:SetColorTexture(0.09,0.09,0.09,1); crossmark:SetVertexColor(0.10,0.10,0.10,1); crossmark:Show(); checkmark:Hide()
        end
    end
...
local function BuildMainFrame()
    ...
    local closeTex=close:CreateTexture(nil,"ARTWORK"); closeTex:SetPoint("CENTER"); closeTex:SetSize(13,13); closeTex:SetTexture(GUI_TEXTURES.."Close.png"); closeTex:SetVertexColor(0.851,0.851,0.851,1); closeTex:SetTexelSnappingBias(0); closeTex:SetSnapToPixelGrid(true)
...
coreEvents:SetScript("OnEvent",function(_,event,arg1) if event=="ADDON_LOADED" and arg1==ADDON_NAME then C.InitDB(); print("|cff33ff99[CC RaidTools]|r v1.0 chargé") end end)
