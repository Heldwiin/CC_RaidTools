...
local function CheckAutoLog()
    ...
    -- Dungeon logging, following MRT's AutoLogging logic:
    -- Mythic = 23, Mythic Keystone = 8. The map restriction avoids automatically
    -- logging old dungeon content when Blizzard reuses the same difficulty IDs.
    if instanceType=="party" and mapID and tonumber(mapID) and mapID>=959 then
        if difficultyID==23 and d.dungeonMythic then
            shouldLog=true
        elseif difficultyID==8 and d.dungeonMythicPlus then
            shouldLog=true
        end
    end
    ...
    elseif not shouldLog and active and startedByAddon then
        LoggingCombat(false)
...
local function BuildUI(f)
    ...
    for _,info in ipairs({
        ...
        {"Héroïque","heroic"},
        {"Mythique","mythic"},
        {"Donjon Mythique","dungeonMythic"},
        {"Donjon Mythique+","dungeonMythicPlus"},
    }) do
